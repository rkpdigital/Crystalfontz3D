/*
 * FILE NAME: config.h - configuration sub-system generic part
 *
 ...
 *
 * NOTES: 	See config_app for application part.
 *
 *
 * CHANGE HISTORY:
 *
...
typedef struct cmdString {				// shared string object
	...
	magic_t magic_end;					// guard to detect string buffer underruns
} cmdStr_t;
...
//	const char *format;				// pointer to formatted print string in FLASH
