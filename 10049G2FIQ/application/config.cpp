/*
 * FILE NAME: config.cpp - configuration sub-system generic part
 *
 ...
 */
/*
 * PURPOSE:	Configuration sub-system generic part
 *
 * NOTES: 	See config.h for a Config system overview and a bunch of details.
 * 		See config_app for application part.
 *
 *
 * CHANGE HISTORY:
 *
...
void cmd_persist(cmdObj_t *cmd)
{
	...
	if (GET_TABLE_BYTE(flags) & F_PERSIST) cmd_write_NVM_value(cmd, cmd->index);
}
...
//+++++ This was turned off in TinyG2... until persistence is implemented
	...
	if ((cmd_read_NVM_value(cmd, cmd->index) == STAT_FILE_NOT_OPEN) || (cmd->value != cs.fw_build))
    {
    ...
    {							// case (2) NVM is setup and in revision
	    ...
	    for (cmd->index=0; cmd_index_is_single(cmd->index); cmd->index++)
        {
            ...
            {
            strcpy(cmd->token, cfgArray[cmd->index].token);	// read the token from the array
            if (cmd_read_NVM_value(cmd, cmd->index) == STAT_OK)
	    	    cmd_set(cmd);
		    }
	    }
...
{
        tempwatch = GET_TABLE_WORD(target);
	*((uint32_t *)tempwatch) = cmd->value;
	cmd->objtype = TYPE_INTEGER;
	return(STAT_OK);
}
		...
		for (uint8_t i=1; i<=COORDS; i++) {
			for (uint8_t j=0; j<AXES; j++) {
				sprintf(cmd.token, "g%2d%c", 53+i, ("xyzabc")[j]);
                                cmd.index = cmd_get_index((const char *)"", cmd.token);
 ...
 * RKP decided to no longer use char_t.
 */
...
cmdObj_t *cmd_add_object(const char *token)  // add an object to the body using a token
{
	...
	for (uint8_t i=0; i<CMD_BODY_LEN; i++) {
        ...
        if ((cmd->index = cmd_get_index((const char *)"",token)) == NO_MATCH) { return (NULL);}
...
cmdObj_t *cmd_add_integer(const char *token, const uint32_t value)// add an integer object to the body
{
	cmdObj_t *cmd = cmd_body;
	for (uint8_t i=0; i<CMD_BODY_LEN; i++) {
		if (cmd->objtype != TYPE_EMPTY) {
...
cmdObj_t *cmd_add_float(const char *token, const float value)	// add a float object to the body
{
	cmdObj_t *cmd = cmd_body;
	for (uint8_t i=0; i<CMD_BODY_LEN; i++) {
		if (cmd->objtype != TYPE_EMPTY) {
...
cmdObj_t *cmd_add_string(const char *token, const char *string) // add a string object to the body
{
	cmdObj_t *cmd = cmd_body;
	for (uint8_t i=0; i<CMD_BODY_LEN; i++) {
		if (cmd->objtype != TYPE_EMPTY) {
...
cmdObj_t *cmd_add_conditional_message(const char *string)	// conditionally add a message object to the body
{
	//if ((cfg.comm_mode == JSON_MODE) && (js.echo_json_messages != true)) { return (NULL);}
    return(cmd_add_string((const char *)"msg", string));
}
 ...
 */
stat_t cmd_read_NVM_value(cmdObj_t *cmd, index_t index)
{
  size_t tempsize;
  int8_t nvm_byte_array[NVM_VALUE_LEN];

    Cfg_fp = fopen(ConfigPathFile, "rb");
    if (!Cfg_fp)
        {
    ...
    fseek(Cfg_fp, index, SEEK_SET);
    tempsize = fread(&nvm_byte_array, 1, NVM_VALUE_LEN, Cfg_fp);
    fclose(Cfg_fp);
    if (tempsize != NVM_VALUE_LEN)
        return(STAT_BUFFER_EMPTY);

    memcpy(&cmd->value, &nvm_byte_array, NVM_VALUE_LEN);
    return (STAT_OK);
}

stat_t cmd_write_NVM_value(cmdObj_t *cmd, index_t index)
{
  int8_t nvm_byte_array[NVM_VALUE_LEN];
...
//    float tmp = cmd->value;
    ...
    Cfg_fp = fopen(ConfigPathFile, "r+b");
    if (!Cfg_fp)
        {
            printf("New Config file %s\n", ConfigPathFile);
            Cfg_fp = fopen(ConfigPathFile, "wb");
            if  (!Cfg_fp)
            {
                printf("Can't Open Config file %s\n", ConfigPathFile);
                return(STAT_FILE_NOT_OPEN);
    ...
    if( 0 != fseek(Cfg_fp, index, SEEK_SET) )
    {
        fclose(Cfg_fp);
        return(STAT_FILE_SIZE_EXCEEDED);
    }

    memcpy(&nvm_byte_array, &cmd->value, NVM_VALUE_LEN);
    fwrite(&cmd->value, 1, NVM_VALUE_LEN, Cfg_fp);
    fclose(Cfg_fp);
...
//#define NVMwr(i,v) { cmd.index=i; cmd.value=v; cmd_write_NVM_value(&cmd, cmd.index);}
//#define NVMrd(i)   { cmd.index=i; cmd_read_NVM_value(&cmd, cmd.index); printf("%f\n", (char *)cmd.value);}
...
//  i = cmd_get_index((const char *)"fb");
//  i = cmd_get_index((const char *)"xfr");
//  i = cmd_get_index((const char *)"g54");
