/*
 * FILE NAME: cycle_homing.cpp - homing cycle extension to canonical_machine
 *
 ...
 */
 /*
 * PURPOSE: homing cycle extension to canonical_machine.
 *
 *
 * CHANGE HISTORY:
 *
...
static stat_t _homing_error_exit(int8_t axis)
{
	...
	if (axis == -2) {
        cmd_add_conditional_message((const char *)"*** WARNING *** Homing error: Specified axis(es) cannot be homed");;
	} else {
		char message[CMD_MESSAGE_LEN];
        sprintf(message, PSTR("*** WARNING *** Homing error: %c axis settings misconfigured"), cm_get_axis_char(axis));
        cmd_add_conditional_message((char *)message);
	}
    cmd_print_list(STAT_HOMING_CYCLE_FAILED, TEXT_INLINE_VALUES, 0); // Was JSON_RESPONSE_FORMAT but it's unused.
