/*
 * report.cpp - Status report and other reporting functions
 *
 ...
 */
/*
 * PURPOSE: Status report and other reporting functions.
 *
 *
 * CHANGE HISTORY:
 *
...
{
    printf(PSTR("{\"er\":{\"fb\":%0.2f,\"st\":%d,\"msg\":\"%s\"}}\n"),
 ...
 * rpt_print_system_working_message()    - system Processing Gcode message
 *
...
void _startup_helper(stat_t status, const char *msg)
{
	...
	cmd_reset_list();
	cmd_add_object((const char *)"fv");		// firmware version
	cmd_add_object((const char *)"fb");		// firmware build
	cmd_add_object((const char *)"hp");		// hardware platform
	cmd_add_object((const char *)"hv");		// hardware version
//	cmd_add_object((const char *)"id");		// hardware ID
	cmd_add_string((const char *)"msg", (const char *)msg);	// startup message
#endif
}
...
void rpt_print_system_working_message(void)
{
	_startup_helper(STAT_OK, PSTR("System Processing Gcode"));
//	if (cfg.comm_mode == TEXT_MODE) { text_response(STAT_OK, (char *)"");}// prompt
}
...
void sr_init_status_report()
{
	...
	index_t sr_start = cmd_get_index((const char *)"", (const char *)"se00");	// set first SR persistence index
	...
	for (uint8_t i=0; i < CMD_STATUS_REPORT_LEN ; i++) {
		...
		sr.status_report_value[i] = -1234567;			// pre-load values with an unlikely number
                cmd->index = sr_start;  // Might add + (i * sizeof(index_t))
		cmd->value = cmd_get_index((const char *)"", sr_defaults[i]);// load the index for the SR element
		cmd_set(cmd);
...
stat_t sr_set_status_report(cmdObj_t *cmd)
{
	...
	for (uint8_t i=0; i<CMD_STATUS_REPORT_LEN; i++) {
		...
		if ((cmd->objtype == TYPE_BOOL) && (fp_TRUE(cmd->value))) {
			status_report_list[i] = i * sizeof(status_report_list);
			cmd->index = sr_start + (i * sizeof(index_t));			// index of the SR persistence location
...
//	if (SysTickTimer_getValue() < sr.status_report_systick) return (STAT_NOOP);
    ...
    cmd_print_list(STAT_OK, TEXT_INLINE_PAIRS, 0); // Was JSON_RESPONSE_FORMAT but it's unused.
	return (STAT_OK);
}
...
{
	sr_populate_unfiltered_status_report();
    cmd_print_list(STAT_OK, TEXT_MULTILINE_FORMATTED, 0); // Was JSON_RESPONSE_FORMAT but it's unused.
	return (STAT_OK);
}
...
stat_t sr_populate_unfiltered_status_report()
{
	...
	index_t sr_start = cmd_get_index((const char *)"", sr_str);// set the index - may be needed by calling function
	cmd = cmd->nx;							// no need to check for NULL as list has just been reset

	for (uint8_t i=0; i<CMD_STATUS_REPORT_LEN; i++) {
                cmd->index = sr_start + (i * sizeof(index_t));
		if (sr.status_report_list[i] == 0 ) { break;}
		cmd_get_cmdObj(cmd);
...
uint8_t sr_populate_filtered_status_report()
{
	...
	cmd->objtype = TYPE_PARENT; 			// setup the parent object
	strcpy(cmd->token, sr_str);
//	index_t sr_start = cmd_get_index((const char *)"", sr_str);// OMITTED - set the index - may be needed by calling function
	cmd = cmd->nx;							// no need to check for NULL as list has just been reset

	for (uint8_t i=0; i<CMD_STATUS_REPORT_LEN; i++) {
                cmd->index = (i * sizeof(index_t));  // Might add sr_start +
		if (sr.status_report_list[i] == 0) { break;}
