/*
 * FILE NAME: plan_line.cpp - acceleration managed line planning and motion execution
 *
 ...
 */
/*
 * PURPOSE: acceleration managed line planning and motion execution.
 *
 *
 * CHANGE HISTORY:
 *
...
static void _calculate_trapezoid(mpBuf_t *bf)
{
	...
	if (bf->length < (bf->head_length + bf->tail_length)) { // it's rate limited
		...
		do {
            ...
            if (++i > TRAPEZOID_ITERATION_MAX) { fprintf(stderr,PSTR("_calculate_trapezoid() failed to converge"));}
