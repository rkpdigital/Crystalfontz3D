/*
 * FILE NAME: gcode_parser.cpp - rs274/ngc Gcode parser
 *
 ...
 */
/*
 * PURPOSE:	This code is the NIST RS274/NGC v3 Gcode parser
 *
 *
 * CHANGE HISTORY:
 *
...
static void _normalize_gcode_block(char *cmd, char **com, char **msg, uint8_t *block_delete_flag);
...
{
    char *cmd = block;					// gcode command or NUL string
    char none = NUL;
...
static void _normalize_gcode_block(char *cmd, char **com, char **msg, uint8_t *block_delete_flag)
{
    char *rd = cmd;				// read pointer
    char *wr = cmd;				// write pointer
...
static stat_t _execute_gcode_block()
{
	...
	switch (gn.next_action) {
		...
		case NEXT_ACTION_STRAIGHT_PROBE: { status = cm_homing_cycle_start(); break;}            // I have no probe G38.2
