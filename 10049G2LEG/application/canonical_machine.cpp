/*
 * FILE NAME: canonical_machine.cpp - rs274/ngc canonical machine.
 *
 ...
 */
/*
 * PURPOSE:	This code is a loose implementation of Kramer, Proctor and Messina's canonical
 *	machining functions as described in the NIST RS274/NGC v3
 *
 * NOTES: The canonical machine is the layer between the Gcode parser and the motion control
 *	code for a specific robot. It keeps state and executes commands - passing the stateless
 *	commands to the motion planning layer.
 *
 *
 * CHANGE HISTORY:
 *
...
void cm_set_model_linenum(uint32_t linenum)
{
    ...
    cmd_add_object((const char *)"n");	// then add the line number to the cmd list
}
...
{
	gm.parameter = seconds;  //Directly add seconds to the FIQ file delay counter.
	mp_dwell(seconds);
	return (STAT_OK);
}
...
{
    cmd_add_string((const char *)"msg", message);	// add message to the response object
}
...
static int8_t _get_axis(const index_t index)
{
    ...
    strcpy(tmp, cfgArray[index].token);			// kind of a hack. Looks for an axis
...
stat_t _get_msg_helper(cmdObj_t *cmd, const char *const msg_array[], uint8_t value)
{
    ...
    return(cmd_copy_string(cmd, (const char *)GET_TEXT_ITEM(msg_array, value)));
}
...
static void _print_axis_flt(cmdObj_t *cmd, const char *format)
{
    ...
    fprintf(stderr, format, cmd->group, cmd->token, cmd->group, cmd->value, units);
}
...
static void _print_axis_coord_flt(cmdObj_t *cmd, const char *format)
{
    ...
    fprintf(stderr, format, cmd->group, cmd->token, cmd->group, cmd->token, cmd->value, units);
}
...
static void _print_pos(cmdObj_t *cmd, const char *format, uint8_t units)
{
    ...
    fprintf(stderr, format, axes[axis], cmd->value, GET_TEXT_ITEM(msg_units, units));
}

void cm_print_am(cmdObj_t *cmd)	// print axis mode with enumeration string
{
    fprintf(stderr, fmt_Xam, cmd->group, cmd->token, cmd->group, (uint8_t)cmd->value,
